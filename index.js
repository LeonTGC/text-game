...
const start = async () => {
  ...
  console.log(
    "you are on a quests to survive, the world is differnt nuclear fallout didnt help an you have woken up from a fridge that somehow kept you alive for 60 years but you havnt aged a day..."
  );
  userSelectWeapon();
};
const userSelectWeapon = async () => {
  console.log(
    "you have to use a weapon to survive, lets see whats laying around...select a weapon"
  );
...
const hillOrSubway = async () => {
  console.log(
    "you have managed to remember how a door works and have now ventured outside"
  );
...
const dogEncounter = () => {
  console.log(
    "you made it, but there is a pack of dogs and they arent very nice"
  );
  console.log(
    "they come at you with teeth and claw n that...one of them had a corkscrew...dont know how they are managing that"
  );
  const result = randomDecision();
  if (result === 0) {
    console.log(
      "luckilly though you sort it out by throwing a bottle of wine at them"
    );
    console.log(
      "the one with the corkscrew was the leader, and go distracted and franticaly tried to open an already opened bottle...idiot"
    );
    changeWeaponOption();
  } else if (result === 1) {
    console.log(
      "they surround you and with once synchronised BARK you get disorientated"
    );
...
const changeWeaponOption = async () => {
  console.log(
    "you start to think...what if...and still have a chance to change your weapon..."
  );
  ...
  if (result) {
    ...
    console.log(
      `and to be honest its quite commendable, so go on...take this shortcut`
    );
    healthChecks();
  } else if (!result) {
...
const offersDrink = async () => {
  console.log(
    "you come across a bunch of people that have body armour that was made out of used sharpies and plastic bottles"
  );
  console.log("stranger: 'do you want a drink eh?'");
  let result = await takeDrink();
  if (result) {
    console.log(
      "you dont know these and the drink had poison in...obviously you loose health"
    );
    healthDecrease();
    changeName();
  } else if (!result) {
    console.log(
      "of course you dont take it, some people would have said yes to that believe it or not"
    );
...
const changeName = async () => {
  console.log(
    "they start to discuss some things...you can't really make out what it is they are saying"
  );
...
const nameRoute = () => {
  ...
  if (checker) {
    ...
    console.log(
      `stranger: 'you're no hero, you are ${superName.villain} the villain!'`
    );
    villainRoute();
  } else {
    console.log("error");
...
const heroRoute = async () => {
  console.log(
    "stranger: 'can you help us, there is a beast over there we cannot be rid of'"
  );
  console.log("they take you over to an old tesco trolley shed");
  console.log(
    "inside you can see a rat, bigger than any of those dogs you saw before"
  );
  console.log("you see next to it a metal barrel with a danger sign on");
  console.log(
    "with some fluid seeping out...so grim, the smell knocks you back"
  );
  healthDecrease();
  ...
  if (result) {
    ...
    if (player.weapon === "deck of cards") {
      console.log(
        `you start to walk away but ${superName.hero} slips and looses health`
      );
      healthDecrease();
    } else {
...
const villainRoute = () => {
  ...
  console.log(
    "you can't help but think you have gotten away with something here"
  );
  healthChecks();
};
...
const healthChecks = () => {
  ...
  console.log(
    "its now a haven, animals can talk and some humans living there too"
  );
